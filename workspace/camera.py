...
class BaslerCameraROITracker(QMainWindow):
    ...
    def show_plot(self):
        ...
        dx_data = [data[3] for data in self.tracking_data]  # dx từ tracking_data
        dy_data = [data[4] for data in self.tracking_data]  # dy từ tracking_data

        # Hiển thị biểu đồ
        plt.figure(figsize=(10, 5))

        # # Biểu đồ quỹ đạo (trajectory)
        # plt.subplot(1, 3, 1)
        # plt.plot(x_coords, y_coords, marker='o', label="Trajectory")
        # plt.title("ROI Trajectory")
        # plt.xlabel("X")
        # plt.ylabel("Y")
        # plt.grid()
        # plt.legend()
        ...
        # # Biểu đồ confidence
        # plt.subplot(1, 3, 2)
        # plt.plot(confidence, color='orange', label="Confidence")
        ...
        # Biểu đồ dx, dy
        plt.subplot(1, 3, 3)
        plt.plot(dx_data, label="dx", color='blue')
        plt.plot(dy_data, label="dy", color='red')
        plt.title("dx, dy vs Time")
        plt.xlabel("Frame")
        plt.ylabel("Change in X/Y")
        plt.grid()
        plt.legend()
